...
GameHud::GameHud()
{
	...
	playerPortrait->AddAnimation("75%", 2, 200, 0, 1);
	playerPortrait->AddAnimation("50%", 2, 200, 0, 2);
	playerPortrait->AddAnimation("25%", 2, 200, 0, 3);
	playerPortrait->AddAnimation("Dead", 2, 200, 0, 4);
	...
	ammoCounterPos = Vector2D(screenX, screenY - 200);
	ammoCounter = std::make_unique<Text>(DEFAULT_FONT, " - / 100", Vector2D(screenX, screenY + 24), SDL_COLOR_SOLID_WHITE, 34);

	infinityAmmoPos = Vector2D(screenX + 50, screenY + 24);
	infiniteAmmoSprite = std::make_unique<Sprite>("infinity.png", screenX + 50, screenY + 24, 19, 11, Vector2D(1, 1));
...
void GameHud::Set_AmmoCounterText(Gun* currentGun)
{
	...
	{
		newPos -= infiniteAmmoSprite->Get_SpriteWidth();
		renderInfinite = true;
	}
	else
	{
